...
const CreateQuestionnaire = () => {
  ...
  const [emailErrors, setEmailErrors] = useState([]); // State for email errors
  ...
  const handleSend = () => {
    ...
    const errors = emailInputs.map((email) => {
      if (!email) {
  ...
  const handleNumberChange = (e) => {
    ...
    setEmailErrors(Array.from({ length: number }, () => "")); // Initialize errors
  };
          ...
          <div className="bg-white rounded-lg p-6 max-h-[500px] max-w-[500px] overflow-y-auto">
            ...
            <div className="flex justify-end mt-4">
              <button
                ...
                disabled={emailErrors.some((error) => error)}
              >
                Submit
              </button>
              <button
