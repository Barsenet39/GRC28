...
import { FaCalendarAlt, FaEye, FaFileAlt, FaProjectDiagram, FaTools } from 'react-icons/fa'; // Importing additional icons

const View = () => {
  const router = useRouter();

  const initialRequests = [
    { id: 'REG/978364', companyName: 'Adama Science and Technology University', date: '2020-12-12', status: 'Accepted', type: 'Project' },
    ...
    { id: 'REG/978373', companyName: 'Digital College', date: '2021-08-30', status: 'Inprogress', type: 'Technical Support' },
  ];
            ...
            <tbody className="text-gray-600 text-sm font-light">
              {requests.map((request) => (
                <tr key={request.id} className="border-b border-gray-200 hover:bg-gray-100">
                  ...
                  <td className="py-3 px-6 text-center font-semibold">{request.status}</td>
                  <td className="py-3 px-6 text-left font-semibold">
                    {request.type === 'Project' ? (
                      <FaProjectDiagram className="inline-block mr-1 text-indigo-500" />
                    ) : request.type === 'Technical Support' ? (
                      <FaTools className="inline-block mr-1 text-orange-500" />
                    ) : (
                      <FaEye className="inline-block mr-1 text-gray-500" />
                    )}
                    {request.type}
                  </td>
                  <td className="py-3 px-6 text-left">
                    {request.status === 'Closed' ? (
                      ...
                      <button
                        className="bg-blue-500 text-white py-1.5 px-3 text-xs rounded-full shadow-md hover:bg-blue-600 transition duration-300 transform hover:scale-105 whitespace-nowrap"
                        onClick={() => handleViewMore('Expired')}
                      >
                        View More
                      </button>
                    ) : (
